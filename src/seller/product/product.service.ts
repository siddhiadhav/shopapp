import { ProductModel, uploadDir } from '@shopverse/common'
...
export class ProductService {
 ...
 y
    ...
    generateBaseUrl(contentType: string, buffer: Buffer){
    ...
    generateProductImages(files: CreateProductDto['files']): Array<{src: string}> {
        ...
        return images.map((file: Express.Multer.File)=>{
            let srcObj= {src: this.generateBaseUrl(file.mimetype, fs.readFileSync(path.join(uploadDir+file.filename)))};
